...
function mood() {
  if (state.sleeping || state.energy < 0.25) return { key: "sleepy", title: "犯困", line: "趴在草地上打小盹。" };
  if (state.hunger > 0.7) return { key: "hungry", title: "饿了", line: "一边散步一边找牧草。" };
...
function nap() { state.sleeping = true; state.energy = clamp(state.energy + 0.22); state.hunger = clamp(state.hunger + 0.08); state.lastAction = "趴在草地上睡着了"; render(); }
...
function tick() {
  if (state.sleeping) { state.energy = clamp(state.energy + 0.015); state.hunger = clamp(state.hunger + 0.01); if (state.energy > 0.95) { state.sleeping = false; state.lastAction = "睡醒后继续散步"; hop(); } }
